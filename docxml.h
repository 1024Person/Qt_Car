...
public:
    ...
    static bool readDataXml(const QString& filePath,vector<vector<QStringList>>& emtName_attrName_attrValue_text,bool isRoot,int parentId=0,const QDomElement parentEmt = QDomElement());
    // 重载版本，如果isRoot是true的话，就调用这个函数，这样能够有效的解耦和
    static bool readDataXml(const QString& filePath,vector<vector<QStringList>>& emtName_attrName_attrValue_text);
